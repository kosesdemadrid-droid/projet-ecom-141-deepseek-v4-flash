    ...
    <main>
      ...
      <div className="mx-auto max-w-7xl px-4 pt-10 sm:px-6">
        <div className="flex flex-wrap justify-center gap-2">
          {['all', ...categories].map((c) => (
            <button key={c} onClick={() => setCat(c)} className={`rounded-full px-4 py-2 text-xs font-bold transition ${cat === c ? 'bg-p text-white shadow-brand' : 'bg-white text-gray-600 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-300'}`}>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-black tracking-tight" style={{ color: 'var(--p2)' }}>{cat === 'all' ? t('featured') : cat}</h2>
            <p className="text-sm text-gray-400">{byCat.length} {t('productCount')}</p>
          </div>
          <a href={`/s/${shop.slug}/products`} className="text-sm font-bold text-p hover:underline">{t('viewAll')} →</a>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="relative overflow-hidden rounded-3xl bg-p p-8 text-white">
            <div className="pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-white/10" />
            <span className="text-xs font-bold uppercase tracking-widest text-white/70">{t('newArrivals')}</span>
            <h3 className="mt-2 text-2xl font-black leading-tight">-25% sur les nouveautés</h3>
            <a href={`/s/${shop.slug}/products?sort=new`} className="mt-5 inline-block rounded-xl bg-white px-6 py-3 text-sm font-black text-p transition hover:-translate-y-0.5">{t('shopNow')} →</a>
          </div>
          <div className="relative overflow-hidden rounded-3xl bg-gray-900 p-8 text-white">
            <div className="pointer-events-none absolute -bottom-10 -right-6 h-44 w-44 rounded-full bg-soft/20" />
            <span className="text-xs font-bold uppercase tracking-widest text-white/50">{t('delivery')}</span>
            <h3 className="mt-2 text-2xl font-black leading-tight">Livraison offerte à Abidjan</h3>
            <p className="mt-2 text-sm text-white/60">dès 50 000 FCFA d'achat</p>
            <a href={`/s/${shop.slug}/products`} className="mt-5 inline-block rounded-xl bg-white px-6 py-3 text-sm font-black text-gray-900 transition hover:-translate-y-0.5">{t('shopNow')} →</a>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-black tracking-tight" style={{ color: 'var(--p2)' }}>{t('newArrivals')}</h2>
            <p className="text-sm text-gray-400">Fraîchement arrivés en boutique</p>
          </div>
          <a href={`/s/${shop.slug}/products?sort=new`} className="text-sm font-bold text-p hover:underline">{t('viewAll')} →</a>
        </div>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
      ...
      {/* Bandeau confiance */}
      <section className="border-y border-gray-100 bg-white py-8 dark:border-gray-800 dark:bg-gray-900">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 text-center sm:grid-cols-4 sm:px-6">
          {[
            ...
            <div key={t2}>
              ...
              <div className="mt-1 text-sm font-bold text-gray-800 dark:text-gray-100">{t2}</div>
