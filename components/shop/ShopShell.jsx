...
export default function ShopShell({ categories, children }) {
  ...
  return (
    ...
    >
      ...
      {/* Bandeau promo */}
      <div className="bg-p text-center text-xs font-bold text-white py-2 px-4">
        {t('promoBanner')}
      </div>

      {/* En-tête */}
      <header className="sticky top-0 z-50 border-b bg-white/90 backdrop-blur-lg dark:bg-gray-900/90 dark:border-gray-800">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6">
          ...
          <Link href={`/s/${shop.slug}`} className="flex min-w-0 items-center gap-2.5">
            {shop.logo ? (
              <Img src={shop.logo} alt="" className="h-9 w-9 shrink-0 rounded-xl object-cover" />
            ) : (
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-white" style={{ background: 'var(--p)' }}>
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            {nav.map((n) => (
              <Link key={n.href} href={n.href} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${isActive(n) ? 'bg-soft text-p' : 'text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white'}`}>
                {n.label}
              </Link>
            ))}
            <div className="mx-1 flex flex-wrap gap-1.5 pl-2">
              {categories.slice(0, 3).map((c) => (
                <Link key={c} href={`/s/${shop.slug}/products?cat=${encodeURIComponent(c)}`} className="rounded-full bg-gray-100 px-3 py-1.5 text-[11px] font-bold text-gray-600 transition hover:bg-soft hover:text-p dark:bg-gray-800 dark:text-gray-300">
                  {c}
                </Link>
              ))}
            </div>
          ...
          <div className="flex items-center gap-1.5">
            <button onClick={() => setSearchOpen(!searchOpen)} className="rounded-lg p-2.5 text-gray-600 transition hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800" aria-label="Recherche">
            ...
            <button onClick={toggleLang} className="hidden rounded-lg px-2.5 py-2 text-xs font-black text-gray-500 transition hover:bg-gray-100 sm:block dark:text-gray-300 dark:hover:bg-gray-800" title="Basculer la langue">
              {lang === 'fr' ? 'FR' : 'EN'} <span className="text-gray-300 dark:text-gray-600">/</span> {lang === 'fr' ? 'EN' : 'FR'}
            </button>
            {shop.social?.whatsapp && (
              <a href={`https://wa.me/${shop.social.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" title="WhatsApp" className="hidden rounded-lg p-2.5 text-emerald-600 transition hover:bg-emerald-50 md:block">
            ...
            <button onClick={() => setCartOpen(true)} className="relative rounded-lg p-2.5 text-gray-700 transition hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800" aria-label="Panier">
              ...
              {cartCount > 0 && (
                <span className="absolute -right-0.5 -top-0.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-p px-1 text-[10px] font-black text-white">{cartCount}</span>
              )}
            </button>
          </div>
        </div>
          ...
          <div className="border-t border-gray-100 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
            <div className="mx-auto flex max-w-2xl items-center gap-2">
              <input
                ...
                className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-gray-400 dark:border-gray-700 dark:bg-gray-800"
              />
              <button onClick={goSearch} className="shrink-0 rounded-xl bg-p px-5 py-3 text-sm font-bold text-white transition hover-bright">{t('search')}</button>
            </div>
          </div>
        )}
        ...
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
            ...
            <div className="md:col-span-2">
              <div className="flex items-center gap-2.5">
                {shop.logo ? (
                  ...
                  <span className="flex h-9 w-9 items-center justify-center rounded-xl text-white" style={{ background: 'var(--p)' }}>{shop.name.slice(0, 1)}</span>
                )}
                <span className="text-lg font-black text-white">{shop.name}</span>
              </div>
              <p className="mt-4 max-w-md text-sm leading-relaxed text-gray-400">{shop.tagline}</p>
              <div className="mt-4 flex items-center gap-2 text-xs font-bold">
                {['🟠 Orange Money', '🟡 MTN MoMo', '🔵 Wave'].map((p) => (
                  <span key={p} className="rounded-full bg-white/10 px-3 py-1.5">{p}</span>
                ))}
              </div>
            </div>
            <div>
              ...
              <ul className="mt-4 space-y-2.5 text-sm">
                ...
                <li><Link href={`/s/${shop.slug}/tracking`} className="transition hover:text-white">{t('orderTracking')}</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="text-sm font-bold text-white">{t('newsletter')}</h4>
              <form onSubmit={newsletter} className="mt-4 flex gap-2">
          ...
          <div className="mt-12 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-6 text-xs text-gray-500 sm:flex-row">
            ...
            <span className="flex items-center gap-2">
              {t('payWith')} :
              {['Orange Money', 'MTN MoMo', 'Wave'].map((p) => <span key={p} className="rounded-md bg-white/5 px-2 py-1 font-bold">{p}</span>)}
            </span>
          </div>
        </div>
      </footer>
