...
async function fetchGist(pathname, init, attempts = 3) {
  let lastErr;
  for (let i = 1; i <= attempts; i++) {
    try {
      const res = await fetch(`https://api.github.com${pathname}`, {
        ...init,
        headers: { Authorization: `token ${GIST_TOKEN}`, 'User-Agent': 'laboutique-ci', ...(init?.headers || {}) },
      });
      if (res.ok) return res;
      const detail = await res.text().catch(() => '');
      lastErr = new Error(`gist ${init?.method || 'GET'} ${res.status} ${detail.slice(0, 200)}`);
      if (res.status < 500 && res.status !== 429) break; // erreurs 4xx : pas de retry
    } catch (e) {
      lastErr = e;
    }
...
async function readGist() {
  const res = await fetchGist(`/gists/${GIST_ID}`);
